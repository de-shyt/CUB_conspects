...
В кэше хранится какой-то кусок памяти, причем поддерживается пространственная локальность. Такой кусок памяти называется *a cache block* или *a cache line*. 

Cache hit -- нужный кусок памяти сохранен в кэше. 

Cache miss -- нужный кусок памяти не сохранен в кэше. Вероятность получить cache miss равна $10\%$.
...
Рассмотрим какой-нибудь адрес. 

Последние два бита -- это смещение байта относительно начала в машинном слове. 

Следующие $i$ битов -- номер сета в кэше, в который это машинное слово будет подгружаться. $i = \log(\texttt{cache size})$. В одном сете лежит один адрес. 

Остальные биты -- адрес в основной памяти. Называется Tag. 
...
Кэш устроен следующим образом: есть бит, тег, данные по адресу. Valid bit показывает, валидны ли данные в текущей строке кэша. 

Хотим обратиться по какому-то адресу.  Из адреса определяем сет, в котором адрес должен жить. Дальше сравниваем тег адреса, по которому хотим обратиться, с тегом, сохраненном в рассматриваемом сете. Если они равны и $\texttt{valid bit} = 1$, возвращаем сохранынные данные. 
